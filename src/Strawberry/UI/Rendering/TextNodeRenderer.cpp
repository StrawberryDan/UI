		...
		: mPipelineLayout([&]() {
		  ...
		  , mGraphicsPipeline([&]() -> Vulkan::GraphicsPipeline {
			  return Vulkan::GraphicsPipeline::Builder(mPipelineLayout, framebuffer.GetRenderPass(), subpassIndex)
					 ...
					 .WithInputBinding(0, 2 * sizeof(Core::Math::Vec2f) + sizeof(std::uint32_t) + 2 * sizeof(Core::Math::Vec2u), VK_VERTEX_INPUT_RATE_INSTANCE)
					 ...
					 .WithInputAttribute(2, 0, offsetof(RenderQueueEntry, glyphAddressPageIndex), VK_FORMAT_R32_UINT)
					 .WithInputAttribute(3, 0, offsetof(RenderQueueEntry, glyphAddressCoordinate), VK_FORMAT_R32G32_UINT)
					 .WithInputAttribute(4, 0, offsetof(RenderQueueEntry, glyphAddressExtent), VK_FORMAT_R32G32_UINT)
