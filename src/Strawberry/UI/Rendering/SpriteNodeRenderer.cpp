	...
	SpriteNodeRenderer::SpriteNodeRenderer(Vulkan::Framebuffer& framebuffer, uint32_t subpassIndex, const Core::Math::Mat4f& projectionMatrix, const Core::Math::Vec2f& contentScale)
		...
		, mPipeline(
			Vulkan::GraphicsPipeline::Builder(mPipelineLayout, framebuffer.GetRenderPass(), subpassIndex)
				...
				.WithInputBinding(0, 4  * sizeof(Core::Math::Vec2f) + sizeof(uint32_t), VK_VERTEX_INPUT_RATE_INSTANCE)
				.WithInputAttribute(0, 0, 0, VK_FORMAT_R32G32_SFLOAT)
				.WithInputAttribute(1, 0, 1 * sizeof(Core::Math::Vec2f), VK_FORMAT_R32G32_SFLOAT)
				.WithInputAttribute(2, 0, 2 * sizeof(Core::Math::Vec2f), VK_FORMAT_R32G32_SFLOAT)
				.WithInputAttribute(3, 0, 3 * sizeof(Core::Math::Vec2f), VK_FORMAT_R32G32_SFLOAT)
				.WithInputAttribute(4, 0, 4 * sizeof(Core::Math::Vec2f), VK_FORMAT_R32_UINT)
